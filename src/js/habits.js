 ...
 */
import { getCurrentWeekData, saveStateToStorage, getWeekKey, STATE, formatDateISO, getUserStorageKey, formatDateDisplay } from './state.js?v=2.8.11';
import { ApiClient } from './apiClient.js?v=2.8.11';
import { escapeHtml, getPriorityPoints } from './utils.js?v=2.8.11';
...
export function renderHabits(habitLogTableBody, totalPointsBadge) {
  ...
  // Include points from completed weekly priority goals (respecting Day View if active)
  const allTodos = weekData.todos || [];
  allTodos.forEach(t => {
    if (t.completed) {
      if (isDayView && selectedDateStr) {
        const isScheduledToday = t.scheduledDate === selectedDateStr ||
